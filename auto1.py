...
def traiter_fichier(fichier):
   ...
   # ----------------- Vérification des colonnes selon opérateur -----------------
        ...
        if operateur_fichier == "MOOV":
            ...
            # Trouver la colonne de commission (ex: "compte:" ou "compte1")
           # col_commission = next((c for c in df.columns if "commission" in c), None)
             # Trouver la colonne de commission (ex: "compte:" ou "compte1")
           # col_comptabiliser = next((c for c in df.columns if "montant_net" in c  ), None)
            ...
            if col_montant:
                df.rename(columns={col_montant: "montant"}, inplace=True)
            # if col_commission:
            #     df.rename(columns={col_commission: "commissoin"}, inplace=True)

            # if col_comptabiliser:
            #     df.rename(columns={col_comptabiliser: "net_a_compt"}, inplace=True)
            if col_date:
                df.rename(columns={col_date: "date_operation"},inplace=True)
            ...
            if "montant" not in df.columns or "commission" not in df.columns:
                ...
                return
            # 🧮 Normalisation des valeurs pour ORANGE
            # if "commissoin" in df.columns:
            #     # convertir en numérique proprement
            #     df["commissoin"] = pd.to_numeric(df["commissoin"], errors="coerce").fillna(0)
            #     # remettre en positif (si valeurs négatives)
            #     df["commissoin"] = df["commissoin"].abs()
        ...
        if operateur_fichier == "CARTE BANCAIRE":
            ...
            # Montant à comptabiliser = montant
            df["montant_a_comptabiliser"] = df["montant"]

            # Ajouter l'opérateur
           # df["opérateur"] = "CARTE BANCAIRE"
            ...
            # # Regrouper par site (si nécessaire)
            # df_group = df.groupby(["opérateur", "site"], as_index=False).sum()

            # # Récupérer la date
            # df_group["date_operation"] = df.groupby(["opérateur", "site"])["date_operation"].first().values

            # # Créer periode = jj-mmm-aa
       ...
       # Arrondir proprement après le groupby
        ...
        for _, row in df_group.iterrows():
            ...
            #ligne Mtn    
              ...
              lignes.append({"Journal":Code, "Date": operation,"Mouvement":"OD", "Compte": comptebrumtn, "Sens Ecriture": "D",  "Montant": montant,"Periode":periode,"Libelle":libelle52})
            ...
            #ligne wave  
                ...
                lignes.append({"Journal":Code, "Date": operation,"Mouvement":"OD", "Compte": comptewa, "Sens Ecriture": "C", "Type Ecriture":"X", "Montant": montant,"Auxil/Analyst": auxi ,"Periode":periode,"Libelle":libelle55 })       
                lignes.append({"Journal":Code, "Date": operation,"Mouvement":"OD", "Compte": comptebruwav, "Sens Ecriture": "D",  "Montant": compta,"Periode":periode,"Libelle":libelle52})
                ...
                lignes.append({"Journal":Code, "Date": operation,"Mouvement":"OD", "Compte": comptecartebancaire, "Sens Ecriture": "C", "Type Ecriture":"X", "Montant": montant,"Auxil/Analyst": auxi ,"Periode":periode,"Libelle":libelle55 })       
                lignes.append({"Journal":Code, "Date": operation,"Mouvement":"OD", "Compte": comptebrumoov_om, "Sens Ecriture": "D",  "Montant": compta,"Periode":periode,"Libelle":libelle52})
