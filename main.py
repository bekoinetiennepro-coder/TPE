    ...
    try:
        ...
        if operateur_fichier == "WAVE":
            ...
            for i, row in df_temp.iterrows():
                ...
                if "site" in line_str:  # MOOV / MTN / ORANGE
                    header_row = i
                    break
        ...
        if operateur_fichier == "MOOV":
            ...
            print("[DEBUG] Colonnes détectées après normalisation :", df.columns.tolist())

            # --- IDENTIFICATION ROBUSTE DE 'nom_marchand' ---
            col_nom_marchand = None
            for col in df.columns:
                if "nom_marchand" in col:
                    col_nom_marchand = col
                    break

            if col_nom_marchand is None:
                print("[ERREUR] Impossible de trouver la colonne Nom marchand dans :", df.columns.tolist())
                return

            # --- EXTRACTION DU SITE ---
            def extraire_site_wave(txt):
                match = re.match(r"^(\d+)-", str(txt).strip())
                return match.group(1) if match else None

            df["site"] = df[col_nom_marchand].apply(extraire_site_wave)

            # --- DÉTECTION DES AUTRES COLONNES ---
            col_montants = next((c for c in df.columns if "montant_brut" in c), None)
            col_commi = next((c for c in df.columns if "frais" in c ), None)
            col_comptab = next((c for c in df.columns if "montant_net" in c or "montant_Net" in c), None)
            
            if col_montants: 
                df.rename(columns={col_montants: "montant"}, inplace=True)
            if col_commi: 
                df.rename(columns={col_commi: "commission"}, inplace=True)
            if col_comptab: 
                df.rename(columns={col_comptab: "montant_à_comptabiliser"}, inplace=True)
            ...
            if manquantes:
                print("[ERREUR] Colonnes manquantes WAVE :", manquantes)
                print("[DEBUG] Colonnes finales :", df.columns.tolist())
                return
            ...
            if "montant_à_comptabiliser" in df.columns:
                # convertir en numérique proprement
                df["montant_à_comptabiliser"] = pd.to_numeric(df["montant_à_comptabiliser"], errors="coerce").fillna(0)
                # remettre en positif (si valeurs négatives)
                #df["montant_à_comptabiliser"] = df["montant_à_comptabiliser"].abs()
        ...
        # Harmonisation si Orange (renommer les colonnes)
        # if operateur_fichier == "ORANGE":
        #     df.rename(columns={
        #         "crédit": "montant",
        #         "Compte1": "commission"
        #     }, inplace=True)
