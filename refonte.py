...
DOSSIER_ARCHIVE = os.path.join(BASE_RESEAU, "Archives_Nouveau")
...
def cumuler_debit(df):
    comptes_cibles = [521650,521650, 521450, 531100,558014]  # comptes à cumuler
    ...
    df_periode = df_debit.groupby(["Date", "Compte","Operateur"])["Periode"].first().reset_index()
    df_group = df_group.merge(df_periode, on=["Date", "Compte","Operateur"], how="left")
...
def exporter_resultat():
    ...
    df_to_export["Montant"] = df_to_export["Montant"].round(2)
