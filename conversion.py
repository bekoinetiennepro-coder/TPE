...
def traiter_fichier(fichier):
   ...
   # ----------------- Vérification des colonnes selon opérateur -----------------
        ...
        if operateur_fichier == "MOOV":
            colonnes_requises = ["opérateur", "site", "montant", "commissoin", "net_a_compt", "date_operation"]
            ...
            if "montant" not in df.columns or "commissoin" not in df.columns:
                print(f"[ERREUR] Colonnes non trouvées pour ORANGE : montant={col_montant}, commissoin={col_commission}")
            ...
            if "montant" not in df.columns or "commissoin" not in df.columns:
                print(f"[ERREUR] Colonnes non trouvées pour WAVE : montant={col_montant}, commissoin={col_commission}")
                print("[DEBUG] Colonnes détectées :", df.columns.tolist())
                return
            # 🧮 Normalisation des valeurs pour ORANGE
            # if "commissoin" in df.columns:
            ...
            colonnes_requises = ["opérateur", "site", "montant", "commissoin", "comptabiliser","terminal"]
       ...
       # Arrondir proprement après le groupby
        ...
        df_group["commissoin"] = df_group["commissoin"].round(2)
        df_group["net_a_compt"] = df_group["net_a_compt"].round(2)
        ...
        for _, row in df_group.iterrows():
            site = row["site"]
            montant = row["montant"]
            commission =  row["commissoin"]
            compta = row["net_a_compt"]
